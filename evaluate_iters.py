...
def evaluate():
    ...
    iter_start = cfg.max_iter - 1000
    iter_end = cfg.max_iter
    eval_step = cfg.msg_iter
    eval_iters = np.arrange(iter_start, iter_end, eval_step)
    ...
    for eval_iter in eval_iters:
        save_pth = osp.join(cfg.respth, 'iter_{:}_'.format(eval_iter) + 'model.pth')
        net.module.load_state_dict(torch.load(save_pth)['state_dict']) if hasattr(net, 'module') else net.load_state_dict(torch.load(save_pth)['state_dict'])
        mIOU = evaluator(net)
        msg = 'iter is {:}, mIOU is: {:.6f}'.format(iter, mIOU)
        logger.info(msg)
        if dist.get_rank() == 0:
            with open(osp.join(osp.join(cfg.respth, 'miou_result.txt'))) as f:
                f.write(msg)
