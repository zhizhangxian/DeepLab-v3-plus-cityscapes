...
class retrain_cell(nn.Module):
    def __init__(self, C_in, C_out, C_skip, C_low, upsample, ops):
        ...
        if C_skip is not None and C_low is not None:
    ...
    def forward(self, x, skip_feature):
        ...
        x = self._op(torch.cat((x, skip_feature), dim=1))
        return x
...
class AutoDecoder(nn.Module):
    def __init__(self, num_classes, args, alphas, betas, gammas, C_low=None):
        ...
        self.output_conv = nn.Conv2d(self._filter_multiplier * self._block_multiplier * self.filter_param_dict[self.net_arch[-1]], self._num_classes, 1, bias=False)
